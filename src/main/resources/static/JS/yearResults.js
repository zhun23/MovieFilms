...
document.addEventListener("DOMContentLoaded", function() {
    const yearQueryElement = document.querySelector("h1 span");
    const yearQuery = yearQueryElement.textContent;

    fetch('http://localhost:8090/api/catalogue/year/' + yearQuery)
        .then(response => response.json())
        .then(data => {
