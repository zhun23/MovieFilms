...
document.addEventListener("DOMContentLoaded", function() {
    const directorQueryElement = document.querySelector("h1 span");
    const directorQuery = directorQueryElement.textContent;

    fetch('http://localhost:8090/api/catalogue/directorSearch?director=' + directorQuery)
        .then(response => response.json())
        .then(data => {
