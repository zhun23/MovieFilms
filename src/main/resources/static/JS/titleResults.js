...
document.addEventListener("DOMContentLoaded", function() {
    const titleQueryElement = document.querySelector("h1 span");
    const titleQuery = titleQueryElement.textContent;

    fetch('http://localhost:8090/api/catalogue/titleSearch?title=' + titleQuery)
        .then(response => response.json())
        .then(data => {
